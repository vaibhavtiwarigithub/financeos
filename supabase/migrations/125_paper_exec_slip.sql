-- Build 4a: realistic paper execution — expected-vs-realized slip tracking.
-- Additive, paper-only. Records the pre-slippage decision price (expected_price)
-- alongside the already-stored slipped fill_price, and stamps the realized slip
-- (fill/expected - 1) at fill time. fill_status is introduced now (single value
-- 'filled') so the later 4b/4c phases (partial fills, next-bar) need no further
-- migration on these tables.
--
-- Nothing here changes sizing, cash movement, gating, or the live path.
-- Old rows keep expected_price/realized_slip_pct/fill_status = null; the
-- Performance-Truth dashboard already renders null as "—" (honesty contract).

alter table paper_trades
  add column if not exists expected_price    numeric,
  add column if not exists realized_slip_pct numeric,
  add column if not exists fill_status       text default 'filled';
...
-- Recreate execute_paper_fill with an additive, defaulted p_expected_price
-- parameter. Dropped + recreated (not CREATE OR REPLACE) because adding a
-- parameter changes the signature; the default keeps the existing 19-arg call
-- site working unchanged. Runs in the migration's transaction, so the function
-- is never observably absent.
drop function if exists execute_paper_fill(
  uuid, text, text, text, numeric, numeric, numeric, text, timestamptz,
  numeric, numeric, numeric, numeric, text, text, text, numeric, numeric, text
);
...
begin
  ...
  -- Realized slip vs the pre-slippage decision price. Null when no expected
  -- price was supplied (e.g. legacy callers) or it is non-positive.
