...
-- nothing else, then DELETED the position row -- and stop_loss/price_target
-- live only on paper_positions. Result: paper_trades.stop_loss was NULL on all
-- 171 closed lots while all 22 open positions carried one, so 19 lots labelled
-- exit_reason='stop_hit' have no recorded stop and 4 of those realized a GAIN.
-- Unauditable, because the number that would settle it was never written down.
--
...
--
-- Version matches supabase_migrations.schema_migrations. The partial-exit stop
-- precedence below is WRONG and is corrected by the next migration; both replay
-- in order to the current production function. Regression test:
...
CREATE OR REPLACE FUNCTION public.execute_paper_exit(
  ...
  p_partial_stop_loss numeric DEFAULT NULL::numeric
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  loop
    ...
    if v_take = v_lot.qty then
      update public.paper_trades set
        ...
        -- Capture the risk levels this exit was judged against, before the
        -- position row carrying them is deleted below.
        stop_loss = coalesce(stop_loss, v_pos.stop_loss),
        take_profit = coalesce(take_profit, v_pos.price_target),
      ...
      insert into public.paper_trades (
        ...
        -- WRONG ORDERING, corrected by the next migration: a residual lot
        -- already carries a stop, so the caller's new p_partial_stop_loss was
        -- silently discarded on every second-and-later partial exit.
        coalesce(v_lot.stop_loss, p_partial_stop_loss, v_pos.stop_loss),
        coalesce(v_lot.take_profit, v_pos.price_target), v_lot.highest_price,
  ...
  insert into public.decision_journal (
    ...
    jsonb_build_object(
      ...
      'stop_loss_at_exit', v_pos.stop_loss, 'price_target_at_exit', v_pos.price_target
    ),
