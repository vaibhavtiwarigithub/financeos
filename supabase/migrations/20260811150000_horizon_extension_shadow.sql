...
CREATE TABLE IF NOT EXISTS horizon_extension_shadow (
  ...
  CONSTRAINT horizon_extension_never_shortens CHECK (effective_exit_day >= LEAST(age_days, horizon_days))
);
...
ALTER TABLE horizon_extension_shadow ENABLE ROW LEVEL SECURITY;

-- Owner may read; nobody may write through PostgREST. Writes come from the
-- service role, which bypasses RLS.
DROP POLICY IF EXISTS horizon_extension_shadow_owner_read ON horizon_extension_shadow;
CREATE POLICY horizon_extension_shadow_owner_read
  ON horizon_extension_shadow FOR SELECT
  TO authenticated
  USING (true);

REVOKE ALL ON horizon_extension_shadow FROM anon;
GRANT SELECT ON horizon_extension_shadow TO authenticated;
...
CREATE TRIGGER horizon_extension_shadow_no_update
  BEFORE UPDATE OR DELETE ON horizon_extension_shadow
