...
export async function PATCH(req: NextRequest) {
  ...
  // These are money limits — owner-only (requireOwner above), never cron.
  ...
  if (max_daily_trades !== undefined && max_daily_trades !== null &&
      (!Number.isInteger(Number(max_daily_trades)) || Number(max_daily_trades) < 1)) {
    return NextResponse.json({ error: "max_daily_trades must be a positive integer or null to clear" }, { status: 400 });
  }
