// SOXS paper cron: entry when flat, monitor+exit when held. Mirrors
// app/api/agents/sqqq/cron/route.ts exactly. Owner-approved 2026-09-23.
// Entry window is 12:00-12:14 ET, continuing the 15-minute offset pattern
...
import { planSoxsEntry, monitorSoxs, type SoxsPolicy } from "@/lib/trading/soxs-lifecycle";
...
async function recordRun(supabase: ReturnType<typeof createServiceClient>, status: "done" | "error", summary: string): Promise<void> {
  ...
  await supabase.from("agent_runs").insert({
    agent_type: AGENT_TYPE, market: "us", status, symbols: ["SOXS"],
...
const SOXS_POLICY: SoxsPolicy = {
  version: "soxs-paper-entry-v1",
...
export async function GET(req: NextRequest) {
  ...
  const result = await runSoxsCron(supabase, now, priorMonitorVerifiedAt);
  await recordRun(supabase, result.runStatus, result.summary);
  return NextResponse.json(result.body, { status: result.httpStatus });
}

async function runSoxsCron(supabase: ReturnType<typeof createServiceClient>, now: number, priorMonitorVerifiedAt: number | null): Promise<RunResult> {
  const { data: existing, error: existingErr } = await supabase
    .from("paper_positions")
    .select("id, qty, avg_cost, current_price, stop_loss, initial_stop_loss, highest_price, price_target")
    .eq("symbol", "SOXS").eq("market", "us").eq("position_role", "soxs_paper").maybeSingle();
  if (existingErr) return fail({ status: "error", reason: `existing_position_query_failed: ${existingErr.message}` });
  if (existing) {
    const monitorQuote = await getQuote("SOXS", supabase);
    const decision = monitorSoxs({
      now,
      ...
      quote: {
        ...
        observedAt: soxsQuoteTime(monitorQuote, now),
      },
      position: {
    ...
    if (decision.action === "data_unavailable" || decision.action === "invalid_position") {
      await reportIssue({
        issueKey: "soxs-monitor-failed", severity: "warn", category: "risk",
        title: `SOXS paper position could not be monitored (${decision.action})`,
    ...
    await resolveIssue("soxs-monitor-failed", supabase);
    ...
    const exitReason = isStop ? "soxs_stop_hit"
      : decision.action === "partial_target" ? "soxs_partial_target"
      : decision.action === "thesis_full" ? "soxs_thesis_invalidated"
      : "soxs_target_hit";
    ...
    if (exitErr) {
      await reportIssue({ issueKey: "soxs-exit-failed", severity: "critical", category: "risk", title: "SOXS paper exit RPC failed", detail: `positionId=${existing.id}: ${exitErr.message}` }, supabase);
  ...
  if (!soxsEntryWindow(new Date(now))) return ok({ status: "no_entry", reason: "outside_entry_window" });
  const [candleResult, quote] = await Promise.all([
    fetchUsCandles("SOXS", NO_AV_FALLBACK).catch(() => ({ candles: [], source: "unavailable" as const })),
    getQuote("SOXS", supabase),
  ]);
  ...
  const { data: lastExit, error: lastExitError } = await supabase
    .from("paper_trades")
    .select("closed_at")
    .eq("symbol", "SOXS").eq("market", "us").eq("position_role", "soxs_paper")
  ...
  const plan = planSoxsEntry({
    ...
    entryWindowOpen: soxsEntryWindow(new Date(now)),
  ...
  if (!plan.ok) {
    await resolveIssue("soxs-entry-cron-failed", supabase);
  ...
  const { data: rpcResult, error: rpcErr } = await supabase.rpc("execute_soxs_paper_fill", {
    p_market: "us", p_currency: "USD", p_symbol: "SOXS",
