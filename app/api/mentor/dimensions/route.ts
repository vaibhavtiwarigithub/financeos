...
export async function GET() {
  ...
  const recent30 = (rows ?? []).filter(r => r.evaluated_at >= new Date(Date.now() - 30 * 86400_000).toISOString().slice(0, 10));
  const averages: Record<string, number> = {};
  for (const dim of DIMS) {
    const vals = recent30.map(r => (r as any)[dim]).filter((v: any) => v != null);
  ...
  const latestObs = (rows ?? [])
    .filter(r => r.observations && (r.observations as any)[weakest])
    .slice(-5)
    .map(r => ({ symbol: r.trade_symbol, date: r.evaluated_at, text: (r.observations as any)[weakest] }));
