...
export const SHADOW_PROGRAMS: readonly ShadowProgramDefinition[] = [
  ...
  },
  {
    ...
    traderBenefit: "A live winner should bank partial profit and let the rest run behind a ratcheting stop, exactly as the paper book does.",
    evidenceSource: "live_exit_ladder_shadow (append-only: the action the engine WOULD have taken, per position per run)",
    currentInfluence: "Shadow-only while live_auto_enabled is false — the monitor evaluates and logs, and submits no order.",
    maximumInfluence: "Once live_auto_enabled is turned on by the owner, the same shared ladder core governs real live exits.",
    activationGate: "Owner flips live_auto_enabled. Before that: shadow rows showing the ladder behaving as paper does on real positions, and the shared-core parity suite green.",
    safetyBoundary: "app_paused and security_locked remain hard stops. Shadow mode writes one evidence row and never reaches an order path. Entry, sizing and stop distance are untouched — only what happens to an already-open position.",
    cronJobs: ["kairos-live-exit-monitor"],
    callAccounting: "zero_incremental",
    owner: "Trading / Risk",
  ...
  },
  {
    ...
    traderBenefit: "Could reduce avoidable losses without widening targets or changing the holding clock.",
    evidenceSource: "exit_stop_shadow_runs",
    currentInfluence: "Measure-only; no money path reads the results.",
    maximumInfluence: "Owner-approved market-local stop challenger after paired cost/FDR and worst-case review.",
    activationGate: "At least 12 effective independent observations, four validation windows, cost/FDR adjustment and owner approval.",
    safetyBoundary: "Only matured entry-eligible labels are used; it cannot alter the live fixed stop or any order.",
    cronJobs: ["kairos-exit-stop-shadow-us", "kairos-exit-stop-shadow-india"],
    callAccounting: "zero_incremental",
    owner: "Trading / Risk",
  ...
  },
  {
    ...
    traderBenefit: "Could identify a better balance of technical, fundamental and trend evidence for each market.",
    evidenceSource: "archetype_ic_runs + shadow_decisions",
    currentInfluence: "Measure-only; no score or weight consumer reads these grades.",
    maximumInfluence: "A validated market-local weighting challenger through the existing replay and owner-promotion path.",
    activationGate: "Stable market-local cohort, independent dates, benchmark-neutral outcomes, sealed validation and owner promotion.",
    safetyBoundary: "Requires entry_eligible=true and direction=long; no automatic weight mutation or trade behavior exists.",
    cronJobs: ["kairos-archetype-ic-us", "kairos-archetype-ic-india"],
    callAccounting: "zero_incremental",
  ...
  },
  {
    ...
    traderBenefit: "Identifies whether improvement belongs in selection, holding period, allocation, risk control or execution costs.",
    evidenceSource: "backtest_experiments where experiment_type=alpha_diagnostic",
    currentInfluence: "Read-only diagnostics; strongest verdict is owner_review and no money-path table is written.",
    maximumInfluence: "Evidence for a separately approved replay or shadow experiment; never direct promotion.",
    activationGate: "A0 data truth must pass, followed by market-local independent dates, complete cohort provenance and owner review.",
    safetyBoundary: "No provider, scorer, PaperTrader, PositionMonitor, promotion, proposal, broker or order import.",
    cronJobs: ["kairos-alpha-diagnostics-us", "kairos-alpha-diagnostics-india"],
    callAccounting: "zero_incremental",
    owner: "Learning / Evidence",
