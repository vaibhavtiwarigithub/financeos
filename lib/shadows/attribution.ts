...
export type AttributionState = "measured" | "collecting" | "not_attributable" | "invalid";
...
export function validateAttributionRow(row: UpgradePathAttributionRow): AttributionValidation {
  ...
  if (!Number.isInteger(row.independent_sessions) || (row.independent_sessions ?? 0) < 1) reasons.push("At least one non-overlapping independent session is required.");
  ...
  if (isFiniteNumber(row.incremental_return_pct) && isFiniteNumber(row.net_incremental_return_pct) && row.net_incremental_return_pct - row.incremental_return_pct > EPSILON) reasons.push("Net incremental return cannot exceed gross incremental return.");
...
export function defaultAttribution(classification: AttributionClass): Pick<UpgradePathAttributionRow, "comparison_type" | "state" | "validity_reason"> {
  return classification === "operational_only"
    ...
    : { comparison_type: classification, state: "collecting", validity_reason: "No immutable matched baseline-versus-variant result has matured yet." };
}
