...
//
// THIS IS NOT A SECOND STALENESS RULE. `assertFreshQuote` remains the authority
// for every quote-shaped decision, and `assertFreshBar` is a thin wrapper that
// calls it. The reason a module is needed at all is that `assertFreshQuote`
// deliberately does NOT compute staleness — it CONSUMES a `stale` flag and says
// so: "The adapters own the staleness rule (market-hours aware). Trust their
// verdict." For a quote that flag comes from `lib/data/quotes.ts`. A raw
// `price_cache` row has no adapter and therefore no flag, so something has to
// supply one. That is `isFreshSessionDate`, and it is the ONLY new predicate
// here — the rejection taxonomy, the provenance checks and the fail-closed
// posture all come from W1 unchanged.
//
// `assessSeries` answers a genuinely different question that has no quote
// analogue: a single quote is one observation, so "is it current" exhausts it,
// but a 21-close vol window or a 260-close benchmark can be perfectly current
// and still too short to be the statistic the caller asked for. Coverage and
// freshness are independent failures and a series must clear both. It shares
// `isFreshSessionDate` with the bar path, so there is still exactly one rule.
//
// The hazard this file must never become is `supabase/functions/_shared/quotes.ts`
// (deleted in W9): a second, weaker rule sitting available for reuse. If you find
// yourself adding a threshold here, put it in the delegation instead.
