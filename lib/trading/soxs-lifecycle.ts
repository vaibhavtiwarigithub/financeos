...
/** Mirrors tqqq-lifecycle.ts / sqqq-lifecycle.ts exactly. Buying shares of
 * SOXS (inverse 3x semiconductors) is an ordinary LONG position in that
 * instrument -- not a short sale -- so the existing long-only
 * decideExitLadder applies unchanged. Owner-approved 2026-09-23.
 */
export interface SoxsPolicy {
  version: string;
...
export interface SoxsQuote { bid: number; ask: number; observedAt: number }
export interface SoxsMonitorQuote { price: number; dayLow: number | null; dayHigh: number | null; observedAt: number }
...
export function planSoxsEntry(input: {
  policy: SoxsPolicy;
  now: number;
  quote: SoxsQuote;
  signalAt: number;
...
export function monitorSoxs(input: {
  now: number; quote: SoxsMonitorQuote; maxQuoteAgeMs: number;
