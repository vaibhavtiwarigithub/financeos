 ...
 * Mirrors soxl-lifecycle.ts exactly — same money-path shape, same pure-module
 * contract — but clamps size to the SHARED SOXL+TQQQ leveraged-sleeve ceiling
 * (leveraged-sleeve-risk.ts) instead of SOXL's semiconductor sector cap: TQQQ
 * tracks the Nasdaq-100, not a semiconductor-sector concentration risk.
 */
export interface TqqqPolicy {
  version: string;
...
export interface TqqqQuote { bid: number; ask: number; observedAt: number }

/** Monitor-side quote contract — see soxl-lifecycle.ts's SoxlMonitorQuote for why
 * this is price + session dayLow/dayHigh, never bid/ask. */
export interface TqqqMonitorQuote { price: number; dayLow: number | null; dayHigh: number | null; observedAt: number }
...
export function planTqqqEntry(input: {
  policy: TqqqPolicy;
  now: number;
  quote: TqqqQuote;
  signalAt: number;
...
export function monitorTqqq(input: {
  now: number; quote: TqqqMonitorQuote; maxQuoteAgeMs: number;
