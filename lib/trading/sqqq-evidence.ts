...
export function sqqqQuoteTime(quote: DeterministicQuote, now: number): number {
...
/** 11:40-11:54 ET — continues the 15-minute offset pattern from SOXL (11:00)
 * and TQQQ (11:20), so no two dedicated doors ever share a cron minute. */
export function sqqqEntryWindow(now: Date): boolean {
  ...
  return hour === 11 && minute >= 40 && minute < 55;
}
