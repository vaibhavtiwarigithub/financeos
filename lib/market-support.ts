...
const REGISTRY: Record<string, MarketSupport> = {
  ...
  "/dashboard/agents":         { level: "full",       note: "Agents run for both markets (India via free Yahoo data); data and currency (₹) follow the header switcher. The 2026-07-06 claim was only PARTLY true — the page shell, Signals, Paper and Weights were scoped, but the Brain, Experiments and Proposals tabs fetched routes that took no market param at all, so the switcher could not reach them (Experiments listed both markets' champions indistinguishably). Those routes are now market-aware and Experiments shows a market badge (fixed 2026-07-16). Live Robinhood trade proposals stay $-denominated regardless of the switcher, since that account is US-only by design." },
