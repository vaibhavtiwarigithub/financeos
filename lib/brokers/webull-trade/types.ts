...
// §"Broker-Neutral Capability Matrix". DEPENDENCY NOTE: the shared type is not
// yet on `main` (no lib/**/*.ts exports it). Per the task brief we define the
// declaration against the spec's shape here; when the shared type lands from
// features/hybrid-stop, replace this local shape with an import and keep the
// webull_trade DECLARATION value below unchanged.
export type Market = "us" | "india";
export type InstrumentType = "equity";
export type AccountMode = "CORE";
...
export interface BrokerProtectiveCapabilities {
  ...
  orders: Partial<
    Record<
      "stop_market" | "stop_limit" | "gtt_limit",
      {
