...
/**
 * NAV from cash plus a mark set — the single definition NAV is built from.
 * Delegates to `computeNav` so this inherits the existing guard that refuses to
 * sum ₹ and $ into one number.
 */
export function navFromMarks(cash: number, marks: PositionMark[]): number {
  return computeNav(cash, marks.map(m => ({ market: m.market, qty: m.qty, price: m.mark })));
}
...
export function reconcilePersistedNav(input: NavReconcileInput): NavReconciliation {
  const nav = navFromMarks(input.cash, input.marks);
  const positionsValue = nav - input.cash;
  ...
  compare("paper_portfolio.nav", nav, input.persistedPortfolioNav,
  ...
  compare("paper_performance.nav", nav, input.persistedPerformanceNav,
  ...
  return { ok: violations.length === 0, navFromMarks: nav, positionsValue, coverage, checks, violations };
}
