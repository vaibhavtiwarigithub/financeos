...
- [~] R16 #6 control plane: reconcile (broker-sync polls getOrder → fill/partial/status write-back + position-mismatch alert) ALREADY EXISTS and now uses the `robinhood` REST adapter; ADDED cancel-on-kill (sync cron cancels resting live orders when a market kill switch trips). REMAINING: live-position protective exits (extend PositionMonitor to the LIVE book — stops/targets/time; best built at live-enable with real fills). Also added market-HOLIDAY guard to `isMarketSessionOpen` (`lib/trading/market-calendar.ts`, US+NSE 2026).
