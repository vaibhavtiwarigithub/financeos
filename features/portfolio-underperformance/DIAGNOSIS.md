...
**Audit correction (2026-08-06):** the pre-audit implementation hard-coded a
`7.5% / 19.2% / 10d` incumbent and called it **LIVE**, although both current
market mandates are `7% / 20% / 10d`. It also selected `price_at_decision` but
entered at a later candle close. More importantly, `PositionMonitor` uses each
fill's resolved stop/target, score exits and partial target fills, none of which
this daily-bar replay reconstructs. Therefore the figures below are **not a
measurement of the live executor** and must not justify an exit-rule change.

The endpoint now uses the logged decision price and current market mandate, but
is explicitly labelled a fixed-geometry mandate proxy. It is diagnostic-only. A
configuration decision requires an execution-faithful portfolio simulation with
actual fills, partial exits, score exits, costs and redeployment; that simulator
does not yet exist.
...
**Audit correction (2026-08-06):** this table was generated by the pre-audit
hard-coded proxy described in §14. It records why per-trade excess cannot rank
rules with different holding periods, but it is **not evidence** that the actual
live trail underperforms SPY or that a candidate trail is better. Use the
performance-truth layer for live performance until the required portfolio
simulation exists.
