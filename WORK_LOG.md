...
| --- | --- | --- | --- | --- |
| Edge/Factor Discovery P1 IC gate (measure-only) | Claude/Opus 4.8 | in_progress | 2026-07-08 | lib/edges/ic.ts (rank-IC vs forward returns by horizon from candles, Newey-West t for overlapping returns, IR, decay, lifecycle classifier) + app/api/agents/edge-ic bounded measure-only route writing edge_ic_history + advisory edge_catalog.status + IC scorecard on /dashboard/edges. NO trading/scoring/sizing change. |
