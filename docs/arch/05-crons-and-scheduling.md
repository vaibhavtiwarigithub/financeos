...
| `kairos-crypto-native-shadow` | Daily 00:15 UTC | `POST /api/agents/crypto-research-shadow` | 24/7, non-executing crypto evidence lane. It uses completed UTC daily candles from Coinbase first, Kraken second, and Alpha Vantage only as a compatibility fallback; it separately records read-only Robinhood onboarding/quote evidence. A current quote cannot imply pair tradability, so all rows remain refused until a verified broker pair-inventory contract exists. No preview, paper fill, or live order occurs. |
