...
> Last updated: 2026-08-07 (Added the two Property collection crons, both weekly on Sunday: `kairos-property-collect` at 10:00 UTC → `POST /api/property/collect`, and `kairos-property-forecast` at 10:30 UTC → `POST /api/property/forecasts`. **Weekly, not daily, on purpose** — FHFA HPI publishes quarterly and BLS LAUS monthly, so a daily poll would spend provider calls re-reading static files. Both routes use the standard cron-secret gate with an owner fallback and are wall-clock bounded. Measure-only: no Property cron output reaches a securities score, order or money path.)
>
