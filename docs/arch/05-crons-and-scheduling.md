...
| `kairos-prewarm-us-postclose` | Weekdays every 2 min, 20:02–20:12 UTC (added 2026-09-10) | `POST /api/agents/prewarm?market=us` | **Post-close price refresh — the only run that can fetch TODAY's bar.** `expectedNewestSession` returns today only *after* the 20:00 UTC close, so the 12:00 UTC prewarm can never see the current session as missing: it reported "134/139 already fresh" while the freshness monitor, running at 20:00, correctly reported 86/103 scopes stale. Nothing refetched between the close and `kairos-position-monitor` at 20:15, so marks, stop checks and target checks ran on the PREVIOUS session's close. Six staggered ticks finish before 20:15; the prewarm is deadline-bounded and priority-ordered (open positions first), so a partial run still refreshes what moves money. |
| `kairos-prewarm-india-postclose` | Weekdays every 2 min, 10:02–10:12 UTC (added 2026-09-10) | `POST /api/agents/prewarm?market=india` | Same defect, same fix for India: the 03:00 UTC prewarm runs before the 03:45 UTC open, while `kairos-position-monitor-india` runs at 11:15 UTC after the 10:00 UTC close. These ticks land between them. |
