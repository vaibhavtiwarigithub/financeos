...
| `kairos-research-us-pm` | Weekdays 18:00 UTC (14:00 ET) | `POST /api/agents/research/cron?market=us` | **Afternoon US research cycle (migration 20260723120000).** Rescores the universe with intraday prices so a stock ripping after the 13:00 UTC morning run gets a fresh score before the close — the input that lets afternoon capital rotation act on the move. |
| `kairos-paper-trade-us-pm` | Weekdays 19:15 UTC (15:15 ET) | `POST /api/agents/paper-trade?market=us` | Afternoon US paper-entry/rotation attempt, 75 min after the PM research run and inside NYSE hours. |
...
| `kairos-earnings-risk-monitor-us` | Weekdays 15:00 UTC (11:00 EDT / 10:00 EST) | `POST /api/agents/earnings-risk-monitor` | Bounded US holdings-only earnings/options shadow. Reads open paper positions and the latest complete per-account live-risk snapshots, merges the PIT cache with one Robinhood calendar call, requests per-symbol/options evidence only for an event inside the holding horizon, and appends `behavior_changed=false` evidence. It never scores, sizes, enters, exits, or calls India options. |
