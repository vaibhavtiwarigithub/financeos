...
> 2026-09-23: **`/api/agents/tqqq/cron` added to `vercel.json`, 4 entries** (mirrors SOXL's own 4 — this doc gains the SOXL entries it was missing in the same edit, both were shipped without a doc update earlier). Owner-approved dedicated paper-trading door for TQQQ (`features/leveraged-etf-and-intraday-execution/FEATURE_ARCHITECTURE.md`'s "TQQQ paper sleeve" section, approved 2026-09-23). Entry+mid-day window `25 15 * 4-10 1-5` / `25 16 * 12,1,2 1-5` (11:25 ET, EDT/EST pair) — offset 15 min after SOXL's 11:00-11:14 window so the two never share a cron minute. After-close monitor `20 20 * 4-10 1-5` / `20 21 * 12,1,2 1-5` (16:20 ET) — shipped from day one, unlike SOXL which only got this after a gap was found post-launch. `/api/agents/soxl/cron` itself runs the same 4-entry EDT/EST pattern at 11:00-11:14 ET (entry+mid-day) and 16:15 ET (after-close monitor), added 2026-09-22/23, `execute_soxl_paper_fill`/`execute_tqqq_paper_fill` RPCs (`position_role='soxl_paper'`/`'tqqq_paper'`). Both crons are isolated doors — see `lib/trading/symbol-policy.ts`'s generic leveraged/inverse block, still in force for every other path. Combined SOXL+TQQQ open notional is capped at 5% of US paper NAV (`lib/trading/leveraged-sleeve-risk.ts`), not 5% each. `leveraged-etf-shadow/collect` (11:00-11:14 ET, EDT/EST pair, existing) now also shadow-observes SQQQ/SOXS (`decision='observe_only'`, no trade door — standing inverse-fund refusal) and records SOXL<->TQQQ trailing-20-session return correlation, informational only.
>
> 2026-09-22: **`kairos-benchmark-scorecard-us-retry2-{dst,standard}` added** (pg_cron jobid 159/160, 21:30/22:30 UTC — 17:30 ET, ~75min after close). Investigation: the US scorecard's `as_of=2026-09-21` snapshot showed `window_end` stuck at `2026-09-11` across every horizon (1M/3M/YTD/1Y all `stale_series`/`insufficient_data`), even though both source series (`paper_performance.nav`, VOO `benchmark_price_observations`) are independently confirmed complete and `source_status='ok'` through 09-21 as of this writing — the underlying data is not the problem. Root cause of that specific run was NOT conclusively identified (could not live-trace the actual 20:45 UTC execution; both `CRON_SECRET` reconciliation and an authenticated owner session were unavailable to manually re-trigger). This is a mitigation (a third, later retry) based on strong circumstantial evidence of a same-day timing/consistency issue, not a proven fix — watch whether `benchmark_scorecard` produces `status='ok'` rows for US after the next trading day before considering this closed. India's equivalent has never shown this pattern.
