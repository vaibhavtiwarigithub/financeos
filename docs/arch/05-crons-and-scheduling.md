...
> Last updated: 2026-07-19 (Added per-market weekend ResearchAgent catch-up. Weekend runs consume idle provider quota over the carry-forward queue but write only `weekend_staged`, `session_validated=false` signals. They never chain PaperTrader/TraderAgent; the next market-session ResearchAgent run must write a fresh validated row.)
...
| `kairos-weekend-research-us` | Saturday + Sunday 15:10 UTC | `POST /api/agents/research/cron?market=us&mode=weekend_catchup` | Scores only US carry-forward queue symbols not already staged for the same completed session. Writes non-executable staged signals; leaves candidates queued for weekday revalidation; never chains a trader. |
| `kairos-weekend-research-india` | Saturday + Sunday 05:10 UTC | `POST /api/agents/research/cron?market=india&mode=weekend_catchup` | India-equivalent catch-up, independently scoped in INR/NSE symbol space. Same non-executable lifecycle. |
