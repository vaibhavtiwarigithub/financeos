...
| `kairos-live-snapshot` | Weekdays 13:00–21:00 UTC every 2h | `POST /api/live-account/refresh-snapshot` | Refreshes the live account book for every CONNECTED cloud MCP broker (Robinhood + Webull via the registry driver `captureAccounts`) → `live_account_snapshots` + `live_performance`. Cloud-native (OAuth vault token, no local machine). Auto-ADDs newly-returned accounts; pruning of removed accounts is deferred (kill-switch-baseline safety) |
