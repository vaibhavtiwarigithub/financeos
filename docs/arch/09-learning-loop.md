...
> **2026-07-28 — PROMOTION PATH CLOSED (owner decision, PROJECT_DECISIONS.md).** Not dormant pending work: measured and closed. Single-factor IC promotion is not reachable at this scale — sigma ~0.27, effective breadth ~17 names, ~180 as-of dates required against ~25 available. A larger universe cannot help (sigma is set by n_eff, not n) and sector-neutral IC would need a 6.1x breadth gain. Build-order steps 5-10 will NOT be built. `POST /api/agents/backtest/promote` is permanently disabled; `strategy_policies` and `backtest_experiments` remain empty governance scaffolding. **EdgeScout/EdgeIC continue as advisory diagnostics and must never be cited as promotion evidence.** Reopening requires a different statistic (portfolio-level cost-adjusted returns) or 5+ years of whole-market liquidity history.
> 2026-07-28 (**measured, promotion still disabled**): the multi-period OOS study (`features/walk-forward-ic-folds` Annex K) found the cross-sectional rank-IC sigma **stable at ~0.27** across four independent periods (spread 1.36x, sd 0.038) — the earlier 0.14-vs-0.27 swing was n=12 noise. Realized sigma is **3.77x** the theoretical 0.0712 at n=200, implying an **effective breadth of ~17 names**: correlation binds, not name count, so enlarging the universe will not reduce sigma and the planned n=400 experiment is **not worth running**. The approved 1C floors assumed sigma ~0.071 and that assumption is REFUTED, not merely unmet — detecting a 0.04 IC at t=2.0 would need ~180 as-of dates against ~25 available. Separately `mom_12_1` at horizon 5 showed alternating fold signs [-1,+1,-1,+1], pooled IC 0.0089, t_HAC 0.32 — no signal at that horizon. Re-deriving the floors against sigma ~0.27 / n_eff ~17 is an architecture decision, not a parameter change.
