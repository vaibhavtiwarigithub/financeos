# Kairos — Tech Stack
> 2026-07-28: **Yahoo daily candles are now a market-agnostic module**, `lib/data/yahoo-candles.ts` (`fetchYahooCandles` + `yahooRange`), moved out of `lib/india-data.ts` where it was named `fetchIndiaCandles`. It was never India-specific — it takes any Yahoo symbol and range. Measured: AAPL `range=5y` = 1254 bars from 2021-07-28, RELIANCE.NS = 1239 bars. This matters because Massive (Polygon-backed), which `resolveCandles()` tries first for US, returns HTTP 403 NOT_AUTHORIZED beyond a 2-year lookback on the current plan, so Yahoo is the free keyless deep-history source for BOTH markets. **Routing US through it is NOT done** — that changes live ResearchAgent scoring inputs and is build-order step 4 in `features/walk-forward-ic-folds/`. Also fixes a defect in the inherited range mapping: it returned `1y` (~247 sessions) for any ask between 366 and 500 days, below the 273 sessions 12-1 momentum needs, so India `mom_12_1` was computed on truncated history at the 420-day default. Boundaries are now each range's own calendar length, so the window returned is always >= the window requested.
