...
>
> Property parcel evidence uses a separate bounded GitHub Actions lane (`.github/workflows/property-evidence.yml`), never a Vercel request. It downloads only after an owner scope exists, filters Maricopa to selected Phoenix ZIPs and TCAD to selected HMAC parcel IDs, uploads no raw artifact, and discards the official archive with the ephemeral runner. Maricopa's audited 44-column R102 header is fail-closed; TCAD follows its official fixed-width layout. These native workers replace generic GitHub scrapers, which remain excluded from the trusted runtime.
>
...
| Python runtime (Vercel + GitHub Actions) | Free Python libraries usable from the TypeScript codebase, **measure-only**. Lane A: `POST /api/py/ic` (`api/py/ic.py`) computes Spearman IC + Newey-West HAC via scipy/statsmodels, gated on the same `x-cron-secret`/`CRON_SECRET` contract as the TS crons (`hmac.compare_digest`, fail-closed when unset). Lane B: weekly GitHub Actions workflow for work exceeding a serverless timeout, dry-run when secrets are absent. `vercel.json` scopes `api/py/*.py` and excludes the Next.js tree from the Python bundle (no tree-shaking, 500MB limit). Verified: Lane A reproduces `lib/edges/ic.ts`'s hand-rolled Newey-West to ~1e-17. Not wired into any score, eligibility, sizing, exit, promotion or broker path. See `features/python-runtime/FEATURE_ARCHITECTURE.md`. | none for Lane A; Lane B needs `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` secrets + `LANE_B_TABLE` variable |
