...
> - **Earnings PIT capture** (`20260715210000`, `…210100`): `earnings_calendar` gains `eps_actual_first, revenue_actual_first, actual_available_at, announcement_session, eps_basis, actual_currency, actual_source, restated_eps, restated_available_at, restated_source, market`; new append-only `earnings_consensus_snapshots` (consensus vintages, RLS auth-read). Data-capture only for a future PEAD/revision edge — no scoring effect. The `post_earnings_drift` archetype was renamed `pre_earnings_proximity_reweight_v1` (it was never real PEAD); `pead_*` reserved.
