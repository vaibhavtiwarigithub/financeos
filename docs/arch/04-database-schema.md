...
> 2026-08-07 (`20260807120000_property_private_scenarios_and_learning.sql`, applied + verified in production): the Property workspace gains its private and evidence tables — `property_assets`, `property_financing_accounts`, `property_scenarios`, `property_imports`, `property_forecast_outcomes`, `property_decision_journal`. All owner-scoped, RLS enabled, and **all grants revoked from `anon` and `authenticated`**: access is server-only through owner-gated routes, so RLS-with-no-policy is deliberate and fail-closed, not an oversight. Private values (addresses, balances, lender terms) are AES-256-GCM encrypted before insert and never enter Postgres as plaintext.
>
