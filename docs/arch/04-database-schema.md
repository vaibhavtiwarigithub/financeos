...
> 2026-09-09 (`manual_trade_guardian_position_ledger` + `learner_runs_live_trade_counts`, **applied and verified** via `information_schema.columns` against the live Supabase project): Manual Trade Guardian Stage 0 adds `agentic_position_ledger` — append-only (blocked by trigger, `UPDATE`/`DELETE` also revoked from `authenticated`/`anon`), one row per detected qty change on account `605420660`: `symbol`, `qty`, `avg_cost`, `source` (`manual` | `agentic`), `matched_broker_order_id` (FK `broker_orders`), `suggested_stop_price`, `stop_proposal_id` (FK `trade_proposals`, unused until Stage 1). RLS: service-role full access, `authenticated` read-only. Separately, `learner_runs` gains `live_trades_closed` (filled-SELL-order proxy count, US only, populated) and `live_win_rate` (NULL always for now — no buy/sell round-trip pairing exists yet, so a win-rate is never fabricated from the proxy). Owner-approved 2026-09-09: paper stays the primary learning evidence, always — this is additive visibility into live activity, not a replacement. See `features/manual-trade-guardian/FEATURE_ARCHITECTURE.md` and `docs/arch/05-crons-and-scheduling.md`.
>
