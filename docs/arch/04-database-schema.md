...
> 2026-09-22 (`20260922000000_leveraged_etf_shadow_observations.sql`, applied and verified in production via Supabase MCP `apply_migration` + `to_regclass`/trigger-count check): created `leveraged_etf_shadow_observations`, which `app/api/agents/leveraged-etf-shadow/route.ts` and `lib/trading/leveraged-etf-shadow.ts` (committed 2026-08-03) referenced but which never existed — the route 503'd `leveraged_etf_shadow_not_deployed` on every call until now. Append-only (immutable-row + no-truncate triggers), `decision` CHECK-locked to `'observe_only'`, `UNIQUE(symbol, market_session)` backing the route's idempotent 23505 handling, no anon/authenticated grants (owner-gated route + service-role client only). No score, sizing, exit, paper, live, or order path reads it. Nothing populates it yet — no cron, no data collector — so the table now exists but is still empty in production.
>
