...
function CustomTooltip({ active, payload, label, cur }: any) {
  if (!active || !payload?.length) return null;
  const val = payload[0].value;
  return (
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: "8px", padding: "8px 12px", fontSize: "12px" }}>
      <div style={{ fontWeight: 700 }}>{label}</div>
      <div style={{ color: val >= 0 ? T.green : T.red, fontWeight: 600 }}>{val >= 0 ? "+" : ""}{cur}{Math.abs(val).toFixed(2)}</div>
    </div>
  );
}

// `trades` arrives already filtered to one market by the caller — the currency
// symbol has to follow it, or India ₹ P&L renders as "$". Values are never summed
// across markets: each render is one market's pool.
export default function PnlBarChart({ trades, market = "us" }: { trades: Trade[]; market?: Market }) {
    ...
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: "12px", padding: "20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
        ...
        <div style={{ fontSize: "13px", fontWeight: 700, color: totalPnl >= 0 ? T.green : T.red }}>
          Total: {totalPnl >= 0 ? "+" : ""}{cur}{totalPnl.toFixed(2)}
        </div>
      </div>
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          ...
          <YAxis tick={{ fontSize: 10, fill: T.muted }} tickLine={false} axisLine={false} tickFormatter={v => cur + v} width={45} />
