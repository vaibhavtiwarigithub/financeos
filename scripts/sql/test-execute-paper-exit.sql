...
DECLARE
  ...
  v_stop numeric; v_tp numeric; v_high numeric; v_qty numeric;
...
BEGIN
  ...
  INSERT INTO public.paper_positions
    (id, symbol, market, currency, qty, avg_cost, current_price,
     stop_loss, price_target, highest_price, opened_at, position_role)
  VALUES (v_pos_id, v_sym, 'us', 'USD', 10, 100, 105, 93.00, 120.00, 125.00, now(), 'alpha');
  ...
  SELECT stop_loss, take_profit, highest_price INTO v_stop, v_tp, v_high
  ...
  IF v_high IS DISTINCT FROM 125.00 THEN
    RAISE EXCEPTION 'CASE1 FAIL: highest_price not captured (got %)', v_high; END IF;
  ...
  INSERT INTO public.paper_positions
    (id, symbol, market, currency, qty, avg_cost, current_price,
     stop_loss, price_target, highest_price, opened_at, position_role)
  VALUES (v_pos_id, v_sym, 'us', 'USD', 10, 100, 130, 93.00, 120.00, 135.00, now(), 'alpha');
  ...
  SELECT stop_loss, take_profit, highest_price, qty INTO v_stop, v_tp, v_high, v_qty
  FROM public.paper_trades WHERE id = v_lot_id;
  ...
  IF v_high IS DISTINCT FROM 135.00 THEN
    RAISE EXCEPTION 'CASE2 FAIL: closed portion high expected 135.00, got %', v_high; END IF;

  SELECT stop_loss, highest_price, qty INTO v_resid_stop, v_high, v_resid_qty
  ...
  IF v_high IS DISTINCT FROM 135.00 THEN
    RAISE EXCEPTION 'CASE2 FAIL: residual high expected 135.00, got %', v_high; END IF;
  ...
  INSERT INTO public.paper_positions
    (id, symbol, market, currency, qty, avg_cost, current_price,
     stop_loss, price_target, highest_price, opened_at, position_role)
  VALUES (v_pos_id, v_sym, 'us', 'USD', 10, 100, 130, 93.00, 120.00, 140.00, now(), 'alpha');
  ...
  RAISE EXCEPTION 'ALL_TESTS_PASSED (3 cases incl. high-water capture; this abort is intentional and rolls everything back)';
END
$test$;
