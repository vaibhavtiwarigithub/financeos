...
$endpoints = @{
  ...
  # Recovery sweep: validates only challengers with no experiment, then may place
  # one passed version per market into non-executing shadow evidence.
  "validation-sweep" = @{ method="POST"; url="$BASE/api/validation/sweep"; headers=@{"x-cron-secret"=$CRON_SECRET;"Content-Type"="application/json"}; body="{}"; timeoutSec=120 }
