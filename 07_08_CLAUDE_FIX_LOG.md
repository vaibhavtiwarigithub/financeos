...
| **S5 / Build 5** Data-provider confidence layer | DEFER_STRATEGIC (PARTIAL) | — | Dimension-level `v_decision_quality` (mig `104`), degraded detection, forced abstention on <2 dims, taint stamped onto trades/orders (mig `116`/`117`) all live | **Measure-only** — learner/validation do NOT yet exclude `excluded_from_learning` rows ("enforcement after golden tests"). No per-feature/provider/last-updated attribution. Enforcement flip is small + high-safety; needs golden tests first. |
...
| **S7 / Build 3** Performance-truth dashboard | DEFER_STRATEGIC (ABSENT) | — | — | Learning page shows only win-rate + NAV + weight bars. No Sharpe/Sortino/max-DD/expectancy/alpha-vs-benchmark/calibration/cost-net/slicing. Raw material exists in ledger (`v_decision_quality`, MAE/MFE, `benchmark_neutral_return`) but is unassembled. Pure-additive read layer; zero live-money risk. |
