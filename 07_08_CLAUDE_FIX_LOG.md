...
| **S6 / Build 4a** Realistic paper execution — expected-vs-realized slip | REAL_FIX_NOW (Phase 4a) | `supabase/migrations/125_paper_exec_slip.sql` (applied), `app/api/agents/paper-trade/route.ts`, `lib/analytics/performance-metrics.ts`, `app/api/agents/performance/metrics/route.ts`, `components/dashboard/PerformanceTruth.tsx`, `tests/performance-metrics.test.ts` | Additive columns `expected_price`/`realized_slip_pct`/`fill_status` on `paper_trades`+`paper_order_events` (**verified applied** in prod via `information_schema`; single `execute_paper_fill` overload confirmed, `p_expected_price` present); RPC now records the pre-slippage decision price and computes `realized_slip_pct = fill/expected − 1` at fill; caller passes pre-slip mid (`price`) on both RPC + legacy-dev paths; new `slip()` estimator (HONESTY CONTRACT: `n`/insufficient, MIN_TRADES=20) + `MODELED_SLIP_PCT=0.05`; owner-gated route emits `slip.realized` vs `slip.modeledPct`; dashboard "Exec Slip (realized)" tile (mobile-first, "too small" when n<20); 21 golden tests pass (3 new); prod build exit 0; endpoint returns owner gate cleanly | Paper-only, zero live-money risk, sizing/cash/gating untouched. Today realized ≈ modeled (same-tick fill) — the tile confirms the assumption; `fill_status` column shipped now so 4b/4c need no further migration on these tables. **Deferred (named):** 4b illiquid rejection + partial fills (needs a per-candidate volume fetch — `DeterministicQuote` has no volume; belongs with Build 6 data layer); 4c next-bar/next-open timing (two-phase pending→fill lifecycle; largest surface, not a live-money hole). |
...
| `125_paper_exec_slip.sql` | Expected-vs-realized exec slip cols + RPC param (S6/Build 4a) | ✅ verified | Yes |
