# FinanceOS Knowledge Base — Master Index
...
Knowledge entries: 6 files | Predictions resolved: 0 | Avg confidence: —
...
|---|---|---|---|
| [firm-blueprints/what-actually-works.md](firm-blueprints/what-actually-works.md) | Distilled lessons from Renaissance, Two Sigma, D.E. Shaw, Man AHL, LTCM | HIGH — sourced from verified research | 2026-06-01 |
...
| [signal-library/proven-signals.md](signal-library/proven-signals.md) | Specific signals with evidence, correlation matrix, regime sensitivity | BUILDING | 0 live predictions yet |

### Event Patterns (agent reads before reacting to news)
| File | Topic | Confidence | Sample Size |
|---|---|---|---|
| [event-patterns/macro-events.md](event-patterns/macro-events.md) | Fed decisions, CPI, jobs reports → typical market reactions | BUILDING | 0 |
...
3. **Evidence accumulates.** Each resolved prediction that touches a knowledge entry increments its `evidence_count`. More evidence → more reliable confidence score.
...
## Core Principles (Non-Negotiable — Never Violate)

These are the lessons from funds that succeeded AND failed. Hardcoded.

1. **Run 5–15 uncorrelated weak signals, never one strong one.** Renaissance wins at 50.75% with thousands of small bets. Concentration kills.
...
7. **The LTCM check:** Before any trade, ask "what correlated event causes all positions to move against me simultaneously? How long can the account survive?" If no answer → no trade.
